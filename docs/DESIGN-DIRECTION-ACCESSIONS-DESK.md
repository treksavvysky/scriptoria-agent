...
- **2026-07-12 — Direction adopted.** Keryx proposal approved by the operator; this document written; doctrine record shelved in The Library (`idea_03001bd6`). Predecessor direction ("The Working Librarian") closed complete.
- **2026-07-12 — Phase 1 done (cortex-os `d0c6d8a`).** Cards gained drift columns (`content_hash`, `byte_size`; in-place migration); `card_from_scriptorium_draft` + source routing in `sync_objects`; `Librarian.checkin` grew a scriptorium branch — the fossil is the draft text verbatim, clarification deliberately empty, `supersedes` honored, card flips to held; the `/checkin` route fires the ingest enrichment pipeline in the background for scriptorium accessions. Same commit window: merged `fionn-fix` (the two long-failing fionn tests were broken tests, not code — cortex-os suite green 13/13 for the first time).
- **2026-07-12 — Phases 2–3 done (shell `05141a1`).** `describe_draft` builds card descriptors (title from first heading, excerpt, sha256, size); `accessions.py` composes the desk; exposed through every door — `card_draft`/`check_in_draft`/`check_out_to_workspace` MCP tools, `cardDraft`/`checkInDraft`/`checkOutToWorkspace` GPT actions (schema re-import needed), and the sub-agent's accessions-desk duty. Check-out auto-creates the target workspace and writes the fossil text as the working copy.
- **2026-07-12 — Phase 4 done.** `scripts/scriptorium_sweep.py` (read-only, opt-in via `.cardable` marker) rides the nightly digest: uncarded drafts, hash drift, gone drafts. All three signals verified live and verified silent on a clean scriptorium. Ruling 3 amendment: gone-draft cards are *reported*, not auto-archived — card archival needs a daemon route that doesn't exist yet; the operator decides.
- **2026-07-12 — Pilot trace mechanized; the direction is complete.** The fable_5 essay carded retroactively through the public door (`held=false`, hash on file). First live check-in: the milestone record `scriptorium_milestones-2026-07-12-accessions-desk-opens-md` went draft → card → fossil → brain enrichment → 5 self-proposed links (including `extends` on this direction's doctrine record) → held card, in one `checkInDraft` call. Curated SYSTEM/completed. Same session, other tracks: `GET /inbox` operator view (`idea_01971d52` → completed) and RITUALS.md manifests (`idea_61b08417` → completed).
